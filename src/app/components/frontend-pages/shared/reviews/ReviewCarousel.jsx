...
function SampleNextArrow(props) {
    ...
    return (
        <Box display="flex" alignItems="center" justifyContent="center"
            className={className}
            sx={{ cursor: 'pointer', position: 'absolute', left: '125px', zIndex: 1, bottom: '45px', right: 0, backgroundColor: (theme) => theme.palette.grey[100], width: '32px', height: '32px', borderRadius: '50%' }}
            onClick={onClick}
        >
...
function SamplePrevArrow(props) {
    ...
    return (
        <Box display="flex" alignItems="center" justifyContent="center"
            className={className}
            sx={{ cursor: 'pointer', position: 'absolute', left: '48px', zIndex: 1, bottom: '45px', right: '60px', backgroundColor: (theme) => theme.palette.grey[100], width: '32px', height: '32px', borderRadius: '50%' }}
            onClick={onClick}
        >
                        ...
                        <Paper variant="outlined" sx={{ borderRadius: '16px'}}>
                            <CardContent sx={{ p: '48px !important' }}>
                                <Typography
                                    ...
                                    mb={3} fontWeight={600} fontSize="24px"
                                ...
                                <Typography variant="body1" lineHeight={1.8} mb={2}>{review.text}</Typography>
                                <Divider />

                                <Typography fontSize="14px" fontWeight={500} ml={5} mt={3}> {activeSlide} / {Reviews.length}</Typography>
                            </CardContent>
                        </Paper>
                    </div>
                ))}
